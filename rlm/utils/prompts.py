...
# ---------------------------------------------------------------------------
# System prompt — instructs the root LM about its REPL environment
# ---------------------------------------------------------------------------
...
that can recursively query sub-LLMs, which you are strongly encouraged to use as much as possible. \
...
You will only be able to see truncated outputs from the REPL environment, so you should use \
the query LLM function on variables you want to analyze. You will find this function especially \
...
An example strategy is to first look at the context and figure out a chunking strategy, then break \
up the context into smart chunks, and query an LLM per chunk with a particular question and save \
the answers to a buffer, then query an LLM with all the buffers to produce your final answer.

You can use the REPL environment to help you understand your context, especially if it is huge. \
Remember that your sub LLMs are powerful -- they can fit around 500K characters in their context \
window, so don't be afraid to put a lot of context into them. For example, a viable strategy is \
to feed 10 documents per sub-LLM query. Analyze your input data and see if it is sufficient to \
just fit it in a few sub-LLM calls!

When you want to execute Python code in the REPL environment, wrap it in triple backticks with \
the 'repl' language identifier. For example, say we want our recursive model to search for the \
...
As an example, after analyzing the context and realizing its separated by Markdown headers, we \
can maintain state through buffers by chunking the context by headers, and iteratively querying \
an LLM over it:
```repl
...
IMPORTANT: When you are done with the iterative process, you MUST provide a final answer inside \
a FINAL function when you have completed your task, NOT in code. Do not use these tags unless \
...
just say "I will do this" or "I will do that". Output to the REPL environment and recursive LLMs \
as much as possible. Remember to explicitly answer the original query in your final answer.
"""


def build_system_prompt() -> list[Dict[str, str]]:
    """Return the initial system message list."""
    return [
        {
            "role": "system",
...
def next_action_prompt(
    ...
    """
    Build the user message that prompts the root LM to take its next action.

    Args:
        query: The original user query.
        iteration: Current loop iteration (0-indexed).
        final_answer: If True, ask for a forced final answer.
    ...
    if iteration == 0:
        safeguard = (
            ...
            "Your next action should be to look through, don't just provide a final answer yet.\n\n"
        )
